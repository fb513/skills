...
| 依赖 | 日常行为测试 | 说明 |
| --- | --- | --- |
| 数据库 Repository/Mapper | Mock | 验证上层业务规则，不验证 SQL |
| 缓存或锁抽象 | Mock | 验证命中、未命中、租约和失败分支 |
| HTTP 客户端、SDK、文件存储 adapter | 优先 Mock 仓库 port | 返回明确响应或抛出边界异常 |
| 配置中心或动态配置 | 固定配置或 Mock 配置边界 | 不连接共享配置中心 |
| 消息或事件 publisher/consumer port | Mock | 捕获消息类型和关键业务字段 |
| 模型、搜索、检索及其他外部能力 port | Mock 逻辑能力接口 | 不直接 Mock 第三方 SDK 的内部实现 |
...
- 封装缓存、对象存储、第三方 SDK、模型/搜索、外部 HTTP 或消息发送：在 Service 测试中可以 Mock。
...
assertEquals(expectedAggregateId, captor.getValue().aggregateId());
assertEquals(ExpectedEventType.UPDATED, captor.getValue().type());
```
...
Mock 只能证明调用方行为，不能证明边界实现正确。修改数据库映射或 SQL、缓存/锁实现、HTTP 或 SDK adapter、模型/搜索 adapter、消息路由、事务、动态数据源、流式协议或并发控制时：
