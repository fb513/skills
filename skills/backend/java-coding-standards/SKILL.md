...
| Database | 即使代码有 bug 也不允许脏数据 | NOT NULL、唯一索引（结构性约束；引用完整性归应用层，见下） |
...
## 数据库约束
- 关键字段加 `NOT NULL` 和默认值
- **业务唯一性用唯一索引保证**，不只靠 Service 层校验 —— 并发下"先查再插"有竞态窗口，只有数据库能保证唯一
- **不加外键、级联、触发器、存储过程**：引用完整性和业务规则由应用层负责。外键跨库失效、强制写入顺序、级联删除影响面不可见，且与软删除语义冲突。详见 `db-change`
- 引用完整性的替代做法：写入前校验引用存在，删除前查下游依赖，跨表一致性由 Service 编排
